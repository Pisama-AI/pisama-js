...
CI's read-only `release-reproduction` job runs the full preparation gates on
Node 22.14.0 and Python 3.11.13, then checks all six archives against
`scripts/reviewed-release-sha256.txt` and uploads the matching artifacts.
That baseline is copied from the independently reviewed R0.8 archive set, not
generated by the build being checked. Future intentional package changes need
fresh artifact review before updating the baseline; unexplained drift must fail.
This job has no OIDC publishing permission or staging environment and does not
replace the external publisher controls, tag binding, or live-client proof.
